(ns apiary.actor
  "ApiaryOperationsActor — the ISCO-08 6123 community apiary actor as a
  `langgraph.graph/state-graph` (ADR-2607011000 / CLAUDE.md Actors
  section). One graph run = one apiary operation request
  ...
  The unconditional invariant: the ApiaryOperationsAdvisor can never
  directly commit a record the ApiaryOperationsGovernor refuses —
  every commit-record! call is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [apiary.advisor :as advisor]
            [apiary.governor :as governor]
            [apiary.store :as store]))

(defn build-graph
  "Build a compiled ApiaryOperationsActor graph. `store` implements
  `apiary.store/Store`. `advisor` implements
  `apiary.advisor/Advisor` (defaults to `mock-advisor`).
